		...
		try{
			...
			while(true){
				...
				if(msgClient.contains("_Size")){
    				...
    				char[] cbuf = new char[ExampleSimple.CHUNK_SIZE];
    				Arrays.fill(cbuf, '\0');
    				
    				int chunkIndex = 0;
    				int numBytes = 0;
    				bufferedReader.skip(partNumber*ExampleSimple.CHUNK_SIZE); // skipping parts not requested, seeking to the part requested
    				
    				if((numBytes = bufferedReader.read(cbuf, 0, ExampleSimple.CHUNK_SIZE))!=-1){
    					//System.out.println("Bytes Num:"+numBytes);
    					//System.out.println("\nBytesRead:");

    					response = String.copyValueOf(cbuf,0,numBytes);
    					//System.out.println(cbufStr);
    					cbuf = new char[ExampleSimple.CHUNK_SIZE];
    					Arrays.fill(cbuf, '\0');
